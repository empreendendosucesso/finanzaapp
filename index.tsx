...
const App = () => {
    ...
    const handleAI = async () => {
        ...
        if (!apiKey) {
            alert('Chave de API não encontrada. Configure o Secret API_KEY no GitHub.');
            return;
        }
        ...
        try {
            ...
            alert('Erro ao gerar insights. Verifique sua cota da API.');
        }
        setLoadingAI(false);
    };
                ...
                <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center bg-slate-100 px-3 py-1.5 rounded-full gap-3 border border-slate-200">
                            ...
                            <span className="text-[10px] font-black uppercase text-slate-500 w-24 text-center tracking-widest">{['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'][currentMonth]}</span>
            ...
            <main className="max-w-6xl mx-auto px-6 mt-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-2 text-emerald-600 mb-2 font-black text-[10px] uppercase tracking-widest"><ArrowUpRight size={14}/>Entradas</div>
                            <div className="text-2xl font-black text-slate-800">R$ {totals.inc.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                        </div>
                        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-2 text-rose-600 mb-2 font-black text-[10px] uppercase tracking-widest"><ArrowDownRight size={14}/>Saídas</div>
                        ...
                        <div className="bg-slate-900 p-6 rounded-3xl text-white shadow-xl">
                            <div className="flex items-center gap-2 mb-2 font-black text-[10px] uppercase opacity-60 tracking-widest">Saldo Atual</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm">
                            <div className="flex items-center gap-2 mb-6 font-black text-[10px] uppercase text-slate-400 tracking-widest">
                                <PieChartIcon size={16} /> Gastos por Categoria
                            </div>
                            <div className="h-[240px] w-full">
                                {chartData.length > 0 ? (
                                    ...
                                    <div className="h-full flex flex-col items-center justify-center text-slate-300">
                                        ...
                                        <span className="text-[10px] font-bold uppercase tracking-widest">Sem dados</span>
                                    </div>
                                )}
                            </div>
                        </div>

                        <div className="bg-gradient-to-br from-indigo-700 to-indigo-900 p-8 rounded-[2rem] text-white relative overflow-hidden flex flex-col justify-between">
                            <div className="absolute top-0 right-0 p-4 opacity-10"><BrainCircuit size={100}/></div>
                            <div className="relative z-10">
                                <div className="flex items-center gap-3 mb-4 font-black text-xs uppercase tracking-widest text-indigo-300">
                                    <Sparkles size={16} /> Insights Gemini
                                </div>
                                {insights ? (
                                    <p className="text-sm leading-relaxed text-indigo-50 font-medium animate-in">{insights}</p>
                                ) : (
                                    <p className="text-sm text-indigo-200/60 font-medium">Pronto para analisar seus gastos deste mês.</p>
                                )}
                            </div>
                    ...
                    <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                            <span className="font-black text-[10px] uppercase text-slate-400 tracking-widest">Extrato</span>
                            <span className="text-[9px] font-bold px-2 py-1 bg-white border border-slate-200 rounded text-slate-500">{filtered.length} transações</span>
                        </div>
                        <div className="divide-y divide-slate-50">
                            {filtered.map((t: any) => (
                                <div key={t.id} className="p-5 px-8 flex items-center justify-between group hover:bg-slate-50">
                                    <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-6">
                                        ...
                                        <button onClick={() => setTransactions(transactions.filter((x: any) => x.id !== t.id))} className="opacity-0 group-hover:opacity-100 p-2 text-slate-300 hover:text-rose-500 transition-all"><Trash2 size={16}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="lg:col-span-4">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm sticky top-24">
                        <h3 className="font-black text-[10px] uppercase text-slate-400 mb-8 tracking-widest">Novo Lançamento</h3>
                        <form onSubmit={handleAdd} className="space-y-5">
                            ...
                            <div className="space-y-1">
                                ...
                                <input type="text" value={form.desc} onChange={e => setForm({...form, desc: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500" required />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[9px] font-black text-slate-400 uppercase ml-2">Valor</label>
                                <input type="number" step="0.01" value={form.val} onChange={e => setForm({...form, val: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-lg font-black outline-none focus:border-indigo-500" required />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[9px] font-black text-slate-400 uppercase ml-2">Categoria</label>
                                <select value={form.cat} onChange={e => setForm({...form, cat: e.target.value})} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-black outline-none">
                                    {(form.type === 'INCOME' ? CATEGORIES.INCOME : CATEGORIES.EXPENSE).map(c => <option key={c} value={c}>{c}</option>)}
                                </select>
                            </div>
                            <button type="submit" className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">Salvar</button>
                        </form>
                    </div>
                </div>
            </main>
